# Skins

Pick a skin at generate time. You can switch anytime without changing data.

| Skin          | Best for                   | Highlight                          | Alias        | 
|---------------|----------------------------|------------------------------------|--------------|
| `typefolio`   | editorial / portfolio      | Right-hand PDF preview             | type         | 
| `kiosk`       | gallery / installation     | Large tap zones, simplified        | presentation |
| `docs-reader` | documentation / webpage    | Simple font matter creation        | docs         |
| `cards-tabs`  | alternative layout         | Clean card grid                    |              |
| `typescatter` | type-forward cards         | Interactive layout                 |              |
| `vite-breeze` | modern “liquid glass”      | HUD + integrated viewer            |              |
| `default`     | neutral baseline           | Minimal surface                    |              |

```bash
prae generate --skin typefolio
prae generate --skin typescatter
prae generate --skin vite-breeze
prae generate
```
See also: [Recipes](/website/docs/docs/recipes.md)
